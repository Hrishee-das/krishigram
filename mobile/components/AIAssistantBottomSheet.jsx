  ...
  return (
    ...
    >
      ...
      >
        ...
        {isDiagnosis ? (
          <View>
            <Text style={styles.diagnosisTitle}>Diagnosis Complete ✨</Text>
            <Text style={styles.diagnosisText}>
              Detected{" "}
              <Text style={{ fontWeight: "bold", color: "#D32F2F" }}>
                {data?.disease_name || "Issue"}
              </Text>
            </Text>
            ...
            <Text style={styles.subText}>
              {data?.what_is_this_disease || "Information pending."}
            </Text>
            ...
            <View style={styles.divider} />
            <Text style={styles.label}>Organic Treatment:</Text>
            <Text style={styles.subText}>
              {data?.organic_treatment || "Consult local expert."}
            </Text>

            <Text style={styles.label}>Chemical Treatment:</Text>
            <Text style={styles.subText}>
              {data?.chemical_treatment || "Not specified."}
            </Text>
          </View>
        ) : (
          <Text
...
export default function AIAssistantBottomSheet({
  visible,
  onClose,
  initialData,
  initialImage,
    ...
    if (visible) {
      ...
      if (initialImage && initialImage !== lastProcessedData.current) {
        ...
        setMessages([
          {
            id: "1",
            text: "Welcome to KrishiGram Doc. Please upload a clear photo of your plant for diagnosis.",
            isUser: false,
          },
        ]);
      }
    ...
    analyzeImage(
      { imageUri },
      {
        onSuccess: (response) => {
          if (response && response.status === "success" && response.report) {
            ...
            setMessages((prev) => [
              ...
              {
                id: Date.now().toString(),
                text: "Internal error in diagnosis report.",
                isUser: false,
              },
            ]);
          }
        ...
        onError: (error) => {
          ...
          if (
            msg.includes("plant") ||
            msg.includes("clear image") ||
            msg.includes("blurry") ||
            msg.includes("work in progress")
          ) {
            Alert.alert("Diagnosis Failed", msg, [{ text: "OK" }]);
            // Remove the "Analyzing..." message if it failed semantic check
            setMessages((prev) =>
              prev.filter((m) => m.text !== "Analyzing plant health..."),
            );
          } else {
            setMessages((prev) => [
              ...prev,
              { id: Date.now().toString(), text: `⚠️ ${msg}`, isUser: false },
            ]);
          }
        },
      },
  ...
  const handleSend = () => {
    ...
    agroChat(
      { query: currentQuery, language: "English" },
      {
        onSuccess: (data) => {
          ...
          if (
            ...
            responseText = "Trial quota reached. Please try again in 1 minute.";
          }
          ...
          >
            ...
            <View style={styles.header}>
              <View style={styles.headerLeft}>
                ...
                <View>
                  <Text style={styles.headerName}>KrishiGram Doc</Text>
            ...
            <View style={styles.inputContainer}>
              ...
              <TouchableOpacity
                style={[
                  styles.micButton,
                  (isAnalyzing || isChatting) && { opacity: 0.6 },
                ]}
                onPress={handleSend}
